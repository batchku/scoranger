...
final class PlaybackTempoTests: XCTestCase {
    ...
    func testTheRangeIsOneToThreeHundred() {
        XCTAssertEqual(PlaybackTempo.minimum, 1)
        XCTAssertEqual(PlaybackTempo.maximum, 300)
        XCTAssertEqual(PlaybackTempo.clamp(0), 1)
        XCTAssertEqual(PlaybackTempo.clamp(-40), 1)
        XCTAssertEqual(PlaybackTempo.clamp(1000), 300)
    ...
    func testTheHandleTravelsLinearlyWithTheNumberUnderIt() {
        ...
        XCTAssertEqual(PlaybackTempo.fraction(forBPM: 300), 1, accuracy: 1e-9)
        XCTAssertEqual(PlaybackTempo.fraction(forBPM: 150.5), 0.5, accuracy: 1e-9)
    }
    ...
    func testADragLandsOnAWholeBeatPerMinute() {
        ...
        XCTAssertEqual(PlaybackTempo.bpm(forFraction: 1), 300)
        XCTAssertEqual(PlaybackTempo.bpm(forFraction: 0.5), 151)
        XCTAssertEqual(PlaybackTempo.bpm(forFraction: -3), 1, "a drag off the end clamps")
        XCTAssertEqual(PlaybackTempo.bpm(forFraction: 9), 300)
    }
    ...
    func testTheSliderAndTheTransportReadTheSameNumber() {
        ...
        XCTAssertEqual(PlaybackTempo.effective(override: 9000, opening: opening), 300,
